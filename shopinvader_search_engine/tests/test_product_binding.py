...
class TestCategoryBinding(TestBindingIndexBase):
    ...
    def _get_categories(self):
        ...
        length = (
            self.env["product.category"].search_count([("id", "parent_of", categ.id)])
            - 1
        )
        while categ:
            res.insert(
    ...
    def test_serialize(self):
        ...
        self.assertEqual(
            self.product_binding.data,
            {
                "id": self.product.id,
                "model": {"name": self.product.product_tmpl_id.name},
                "main": False,
                "name": self.product.full_name,
                "short_name": "Steel, Black",
                "variant_count": 4,
                "categories": self._get_categories(),
                "sku": "FURN_0097",
                "variant_attributes": {"color": "Black", "legs": "Steel"},
                "price": {},
            },
        )
